...
<?php if(isset($_POST['kaydet'])){ $sef = temizle($_POST['baslik']);
...
$kayit = $db->prepare("update sayfalar set sira=?,baslik=?,icon=?,icerik=?,sefurl=? where id=?"); 
$kayit->execute(array($_POST['sira'], $_POST['baslik'], $_POST['icon'], $_POST['icerik'], $sef, $id));

echo '<div class="alert alert-success">Düzenleme kaydedildi..</div>'; echo '<meta http-equiv="refresh" content="2">'; } } ?>
<form method="post">
<?php foreach($db->query("select * from sayfalar where id='".$id."'") as $aaa){ ?>
		...
		<div class="control-group">											
			...
			<div class="controls">
			<input type="text" class="span6" name="icon" value="<?php echo $aaa['icon']; ?>" placeholder="file, edit, bookmark">
		...
		<div class="control-group">											
			...
			<div class="controls">
			<textarea name="icerik" class="span6" rows="5"><?php echo $aaa['icerik']; ?></textarea>			
			</div>				
		</div>		

		<div class="form-actions">
			<button type="submit" name="kaydet" class="btn btn-primary"><i class="icon-edit"></i> Değişiklikleri Kaydet</button> 
