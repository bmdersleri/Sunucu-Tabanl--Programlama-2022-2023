<?php session_start(); define("include",true); include("../assets/blog_conf.php");
if(isset($_SESSION['furtherup'])){ header("Location: home.php"); }else{ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
	...
	<div class="navbar-inner">
		<div class="container">
			...
			<a class="brand" href="index.php">
				<?php echo $print['siteadi_title']; ?> Yönetim Paneli				
			</a>		
...
$kontrol = $db->prepare("select * from yonetim where username=?"); $kontrol->execute(array($username)); if($kontrol->rowCount()=="0"){ echo '<div class="alert alert-danger">Girdiğiniz bilgiler hatalı!</div>'; }else{ $row = $kontrol->fetch(PDO::FETCH_ASSOC);
 ...
 if($username==$dbusername&&$parola==$dbpassword){
	...
	$_SESSION['username']="$username"; $_SESSION['furtherup']="$id";
